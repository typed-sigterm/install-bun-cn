#!/usr/bin/env bash
# 基于 https://github.com/oven-sh/bun/blob/bun-v1.2.2/src/cli/install.sh
...
if [[ ${OS:-} = Windows_NT ]]; then
  if [[ $platform != MINGW64* ]]; then
    echo "Error: You are using Windows, but running install script for Linux/MacOS" >&2
    echo "Info: Please use install.ps1 instead of install.sh" >&2
    exit 1
  fi
fi
...
cnpm_base="https://registry.npmmirror.com/@oven/bun-$target"
...
if [[ $# = 0 ]]; then
    metadata=$(curl -s $cnpm_base)
    latest=${metadata#*\"latest\":\"}
    latest=${latest%%\"*}
    bun_uri=$cnpm_base/-/bun-$target-$latest.tgz
else
    bun_uri=$cnpm_base/-/bun-$target-$1.tgz
fi
