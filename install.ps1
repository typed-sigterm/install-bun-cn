#!/usr/bin/env pwsh
# 基于 https://github.com/oven-sh/bun/blob/bun-v1.2.2/src/cli/install.ps1
...
function Install-Bun {
  ...
  $Target = if ($IsBaseline) { "bun-windows-$Arch-baseline" } else { "bun-windows-$Arch" }

  # Use npmmirror instead:
  $cnpm_base = "https://registry.npmmirror.com/@oven/$Target"

  if ($Version -eq "latest") {
    $metadata = Invoke-RestMethod -Uri $cnpm_base
    $latest = $metadata."dist-tags"."latest"
    $URL = "$cnpm_base/-/$Target-$latest.tgz"
  } else {
    $URL = "$cnpm_base/-/$Target-$Version.tgz"
  }

  $TgzPath = "${BunBin}\$Target.tgz"
  ...
  if (-not $DownloadWithoutCurl) {
    curl.exe "-#SfLo" "$TgzPath" "$URL" 
  }
  if ($DownloadWithoutCurl -or ($LASTEXITCODE -ne 0)) {
    Write-Warning "The command 'curl.exe $URL -o $TgzPath' exited with code ${LASTEXITCODE}`nTrying an alternative download method..."
    try {
      # Use Invoke-RestMethod instead of Invoke-WebRequest because Invoke-WebRequest breaks on
      # some machines, see 
      Invoke-RestMethod -Uri $URL -OutFile $TgzPath
    } catch {
      Write-Output "Install Failed - could not download $URL"
      Write-Output "The command 'Invoke-RestMethod $URL -OutFile $TgzPath' exited with code ${LASTEXITCODE}`n"
      return 1
    }
  }
  ...
  if (!(Test-Path $TgzPath)) {
    Write-Output "Install Failed - could not download $URL"
    Write-Output "The file '$TgzPath' does not exist. Did an antivirus delete it?`n"
    return 1
  }

  try {
    tar.exe -xzf $TgzPath -C $BunBin
    if (!(Test-Path "${BunBin}\package\bin\bun.exe")) {
      throw "The file '${BunBin}\package\bin\bun.exe' does not exist. Download is corrupt or intercepted Antivirus?`n"
    }
  } catch {
    Write-Output "Install Failed - could not untar $TgzPath"
    Write-Error $_
    return 1
  }

  Move-Item "${BunBin}\package\bin\bun.exe" "${BunBin}\bun.exe" -Force

  Remove-Item "${BunBin}\package" -Recurse -Force
